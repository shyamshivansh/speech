const officegen = require('officegen');
const fs = require('fs');
var path = require('path');

function templateToWord() {
    let docx = officegen('docx');

    let headoption={
        color: "gray",
        b: true,
        align: 'center',
        shd: {
            fill: "7F7F7F",
            themeFill: "text1",
            "themeFillTint": "10"
        }
      };
    let bodyoption={
        color: "gray",
        b: false,
        align: 'center',
        shd: {
            fill: "000088",
            themeFill: "text1",
            "themeFillTint": "-10"
        }
    }  

    // Table 1
    let table = [
        [{  type: "text",
            val: "Patient ID",
            opts:headoption,
        }, {
            type: "text",
            val: "Patient Name",
            opts:headoption,
        }, {
            type: "text",
            val: "Date",
            opts:headoption,
        }, {
            type: "text",
            val: "Study",
            opts:headoption,
        }],
        [{val:5469,opts:bodyoption}, {val:'TULSI BAI 65 YF',opts:bodyoption}, {val:'30-January-2024',opts:bodyoption}, {val:"CT BRAIN",opts:bodyoption}],
    ];

    let tableStyle = {
        indent: 100,
        tableColWidth: 2000,
        tableFontFamily: 'Comic Sans MS',
        font_size:4,
        columns: [{ width: 100 }, { width: 250 }, { width: 450 }, { width: 100 }],
    };

    docx.createTable(table, tableStyle);

    // Table 2
    let table1 = [
        [{   type: "text",
            val: "Gender",
            opts:headoption,
        }, {
            type: "text",
            val: "Modality",
            opts:headoption,
        }, {
            type: "text",
            val: "Ref Doctor",
            opts:headoption,
        }, {
            type: "text",
            val: "Age",
            opts:headoption,
        }],
        [{val:"F",opts:bodyoption}, {val:'CT\SR',opts:bodyoption},{val:"DR. MANOJ MURALI",opts:bodyoption}, {val:'065Y',opts:bodyoption}],
    ];

    let tableStyle1 = {
        indent: 100,
        tableColWidth: 2000,
        tableFontFamily: 'Comic Sans MS',
        font_size:4,
        columns: [{ width: 100 }, { width: 250 }, { width: 450 }, { width: 100 }],
    };

    docx.createTable(table1, tableStyle1);

    // containt Header
  let Header = docx.createP({align:"center"});
  Header.addLineBreak();
  Header.addText("CT SCAN OF BRAIN ",{bold:true,font_size:20});
  Header.addLineBreak();

  // container STUDY PROTOCOL
  let Study=docx.createP({align:"left"});
  Study.addText("STUDY PROTOCOL:",{bold:true,font_size:15});
   Study.addLineBreak();
   Study.addText("Sections of appropriate thickness were done at adequate intervals in state of art multi detector CT scanner so as to cover the entire region. \nPost processing was done on advance work station to obtain multiplaner sections.",{bold:true,font_size:10});
   Study.addLineBreak();
   
   // observation 
   let Observation=docx.createP({align:"left"});
  Observation.addText("OBSERVATION:",{bold:true,font_size:15});
   Observation.addLineBreak();
   Observation.addText(`•Cerebral parenchyma appears normal in CT attenuation with preserved gray white matter differentiation.\n•No evidence of focal lesion seen.\n•Ventricular system is within normal limits bilaterally symmetrical.\n•Cisternal spaces are normal.\n•Bilateral basal ganglia, internal capsule, corpus callosum and thalamus are normal in attenuation.\n•There is no shift of midline structures with normal midline interhemispheric fissure.\n•Cerebellum and brainstem appear normal.\n•No evidence of any intra axial or extra axial hemorrhage is seen.\n•Bony skull vault appears normal.\n•Cranio-cervical junction is intact.\n•Bilateral mastoid and visualized temporal bone are normal.\n•Visualized paranasal sinuses are normal.\n•Soft tissue scalp is normal.\n•Bilateral visualized orbits and eye globes are normal.\n•Sellar & parasellar structures are normal.\n•Bilateral cerebello-pontine angles are normal.
  `,{font_size:10})
  Observation.addLineBreak();

  //Impression
  let Impression=docx.createP({align:"left"});
  Impression.addText("IMPRESSION:",{bold:true,font_size:15});
  Impression.addLineBreak();
  Impression.addText(`CT findings are suggestive of:\n• No significant abnormality seen in brain parenchyma.\n• No infarct / SOL / hemorrhage is seen.`,{font_size:10})
  Impression.addLineBreak();
  Impression.addLineBreak();

  //smalltext
  let smalltext=docx.createP({align:"left"});
  smalltext.addText("Please correlate clinically and with related investigations; it may be more informative",{font_size:8});
  smalltext.addText("\nThis report is based on digital DICOM images provided via the internet without identification of the patient, not on the films /plates provided to the patient.",{font_size:12,italic:true,bold:true})
  smalltext.addLineBreak();

  //Happy Recovery
  let Hr=docx.createP({align:"center"});
  Hr.addText("WISH YOU A SPEEDY RECOVERY",{bold:true,font_size:15});
  Hr.addLineBreak();
  Hr.addLineBreak();
  Hr.addText("Thanks for Referral",{bold:true,font_size:12})

  //Disclaimer
  let Disclaimer =docx.createP({align:"centre"});
  Disclaimer.addText("Disclaimer:-",{bold:true,font_size:10});
  Disclaimer.addText("It is an online interpretation of medical imaging based on clinical data. All modern machines/procedures have their own limitation. If there is any clinical discrepancy, this investigation may be repeated or reassessed by other tests. Patient's identification in online reporting is not established, so in no way this report can be utilized for any medico legal purpose. In case of any discrepancy due to typing error or machinery error please get it rectified immediately.",{font_size:8});
  Disclaimer.addLineBreak();

  //Sign Image and DoctorInfo
  let Image=docx.createP({align:"left"});
  Image.addImage(path.resolve(__dirname, 'sunilJisign.jpeg'));
  Image.addLineBreak();
  Image.addText("Dr. Sunil Jakhar",{font_size:10});
  Image.addText("\nMD (Radio-Diagnosis)",{font_size:10});
  Image.addText("\nVolente Teleradiology",{font_size:10});


  const out = fs.createWriteStream("brain.docx");
    docx.generate(out);
}

templateToWord();
